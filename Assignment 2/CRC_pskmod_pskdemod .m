...
for i=1:3
    ...
    for j=0:times
       %% input +reshape
        ...
        modulatedSymbol = qammod(bitToSymbolMapping,M);
        
       %% Use Ricianchan Chanel 
        SNR = [];
        BER = [];
        for snr =0:15
            SNR = [SNR snr];
...
%             fadedSignal = absForm .* modulatedSymbol;
             ...
             chan = ricianchan(ts,fd,kk);
            fadedSignal = filter(chan,modulatedSymbol);
            
            noisySymbols = awgn(fadedSignal,snr,'measured');
            %% use qamdemod
            demodulatedSymbol = qamdemod(noisySymbols,M);
